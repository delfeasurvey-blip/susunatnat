-- =============================================
-- NatNat Fresh Milk - Complete Database Setup
-- =============================================
-- Combines: 001_initial_schema.sql + 002_auth_setup.sql
...
-- =============================================
-- Table: admin_profiles (for Supabase Auth)
-- =============================================
...
-- Mitra SPPG: public read, anon + authenticated admin write
...
-- Articles: public read, anon + authenticated admin write
create policy "Public read articles" on public.articles for select using (true);
create policy "Authenticated admins can write articles" on public.articles for all 
  using (auth.role() in ('anon', 'authenticated') and exists (select 1 from public.admin_profiles where admin_profiles.id = auth.uid()))
  with check (auth.role() in ('anon', 'authenticated') and exists (select 1 from public.admin_profiles where admin_profiles.id = auth.uid()));

-- Lab Reports: public read, anon + authenticated admin write
...
-- Orders: public read, anon + authenticated admin write
create policy "Public read orders" on public.orders for select using (true);
create policy "Authenticated admins can write orders" on public.orders for all 
  using (auth.role() in ('anon', 'authenticated') and exists (select 1 from public.admin_profiles where admin_profiles.id = auth.uid()))
  with check (auth.role() in ('anon', 'authenticated') and exists (select 1 from public.admin_profiles where admin_profiles.id = auth.uid()));

-- Deliveries: public read, anon + authenticated admin write
...
-- Tickets: public read, anon + authenticated admin write
...
-- Promos: public read, anon + authenticated admin write
...
-- Landing Settings: public read, anon + authenticated admin write
...
-- About Settings: public read, anon + authenticated admin write
...
-- Admin Profiles: anyone can check if admin exists, authenticated can insert/read own
